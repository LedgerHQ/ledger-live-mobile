...
export default function BuyOption(props: BuyOptionProps) {
    ...
    return (
        <TouchableOpacity
            style={[styles.body, {
                borderColor: isActive ? "rgb(100, 144, 241)" : "rgba(20, 37, 51, 0.1)",
            }]}
            onPress={onPress}
        >
            <View style={styles.header}>
                <View style={styles.icon}>{icon}</View>
                <LText semiBold style={styles.name}>{name}</LText>
            </View>
            <ProviderBullet text={t("exchange.buy.bullets.whereToBuy")} />
            <ProviderBullet text={`${supportedCoinsCount}+ ${t("exchange.buy.bullets.cryptoSupported")}`} />
