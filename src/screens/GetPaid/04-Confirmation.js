...
export default function getPaidConfirmation({ navigation, route }: Props) {
  ...
  const { onSuccess, onError, message, fcm, amount } = route.params;

  const currency = account ? getAccountCurrency(account) : null;

  const deeplink = useMemo(
    () =>
      "ledgerlive://pay?currency=" +
      currency?.id +
      "&data=" +
      Buffer.from(
        JSON.stringify({
    ...
    [amount, message, account?.freshAddress, currency?.id, fcm],
  );
  ...
  const verifyOnDevice = useCallback(
    async (device: Device): Promise<void> => {
      ...
      sub.current = (mainAccount.id.startsWith("mock")
        ...
        : getAccountBridge(mainAccount).getPaid(mainAccount, {
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
        ...
        <View style={styles.container}>
          <Touchable event="QRZoom" onPress={onZoom}>
            {width < 350 ? (
              ...
              >
                <QRCode size={QRSize} value={deeplink} ecl="H" />
              </View>
            )}
          </Touchable>
          <View>
            <LText style={styles.addressTitle} color="grey">
              <Trans i18nKey="transfer.getPaid.address" />
            </LText>
          </View>
          ...
          <View style={styles.address}>
            <DisplayAddress address={deeplink} verified={verified} />
          </View>
          {mainAccount.derivationMode === "taproot" ? (
            <View style={styles.taprootWarning}>
              <Alert type="warning">
                <Trans i18nKey="transfer.getPaid.taprootWarning" />
              </Alert>
            </View>
          ) : null}
          ...
          <View style={styles.copyLink}>
            <CopyLink
              ...
              string={deeplink}
              replacement={<Trans i18nKey="transfer.getPaid.addressCopied" />}
            >
              <Trans i18nKey="transfer.getPaid.copyAddress" />
            </CopyLink>
            <View style={styles.copyShare}>
              <ShareLink value={deeplink}>
                <Trans i18nKey="transfer.getPaid.shareAddress" />
              </ShareLink>
            </View>
          </View>
        </View>
      ...
      {verified && (
        ...
        >
          ...
          />
          <Button
            event="getPaidVerifyAgain"
            containerStyle={styles.bigButton}
            type="primary"
            title={<Trans i18nKey="transfer.getPaid.verifyAgain" />}
            onPress={onRetry}
          />
        </View>
      )}
      ...
      >
        <View style={[styles.qrZoomWrapper, { backgroundColor: "#FFF" }]}>
          <QRCode size={width - 66} value={deeplink} ecl="H" />
        </View>
      </ReactNativeModal>
      <BottomModal
        id="getPaidConfirmationModal"
