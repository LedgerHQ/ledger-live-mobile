...
export default function ValidationSuccess({ navigation, route }: Props) {
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <TrackScreen category="SolanaOptIn" name="ValidationSuccess" />
      <PreventNativeBack />
      <ValidateSuccess
        onClose={onClose}
        ...
        title={
          <Trans
            i18nKey={`solana.optIn.flow.steps.verification.success.title`}
            values={{ token: token?.name }}
          />
        }
        description={
          <Trans
            i18nKey="solana.optIn.flow.steps.verification.success.text"
            values={{ token: token?.name }}
          />
        }
      />
