...
export default function SelectToken({ navigation, route }: Props) {
  ...
  invariant(
    mainAccount && mainAccount.currency.family === "solana",
    "solana Account required",
  );
  ...
  const onNext = useCallback(
    (tokenId: string) => {
      navigation.navigate(ScreenName.SolanaOptInSelectDevice, {
        ...route.params,
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: colors.background }]}>
      <TrackScreen category="OptInFlow" name="Started" />
        ...
        <View style={styles.modal}>
          ...
          <View style={styles.infoRow}>
            <LText style={[styles.warnText, styles.title]} semiBold>
              <Trans
                i18nKey={`solana.optIn.flow.steps.selectToken.warning.title`}
              />
            </LText>
            <LText style={styles.warnText} color="grey">
              <Trans
                i18nKey={`solana.optIn.flow.steps.selectToken.warning.description`}
