...
import { AccountBridge } from "@ledgerhq/live-common/lib/types";
import useBridgeTransaction from "@ledgerhq/live-common/lib/bridge/useBridgeTransaction";
//import {} from "@ledgerhq/live-common/lib/families/solana/logic";
...
function DelegationSelectValidator({ navigation, route }: Props) {
  ...
  invariant(
    account && account.solanaResources,
    "solana account and resources required",
  );

  const mainAccount = getMainAccount(account, undefined);
  const bridge: AccountBridge<Transaction> = getAccountBridge(
    account,
    undefined,
  );
  ...
  invariant(transaction.family === "solana", "must be a solana transaction");
  ...
  const validatorsWithMetaSearched = useMemo(() => {
    return validatorsWithMeta.filter(({ meta, validator }) => {
      return (
        meta.name?.startsWith(searchQuery) ||
        validator.voteAccAddr.startsWith(searchQuery)
      );
    });
  }, [validatorsWithMeta, searchQuery]);

  const chosenVoteAccAddr = transaction.model.uiState.delegate?.voteAccAddress;

  const sections = useMemo(
    () =>
      validatorsWithMetaSearched
        .reduce(
          (data, validatorWithMeta) => {
            const isSelected =
              validatorWithMeta.validator.voteAccAddr === chosenVoteAccAddr;
            if (isSelected) data[0].data.push(validatorWithMeta);
            else data[1].data.push(validatorWithMeta);
            return data;
          },
          [
            {
    ...
    [chosenVoteAccAddr, validatorsWithMetaSearched],
  );
