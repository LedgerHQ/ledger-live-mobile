...
export default function PlatONCustomFees({ navigation, route }: Props) {
  const { colors } = useTheme();
  const { account, parentAccount } = useSelector(accountScreenSelector(route));
  const { transaction } = route.params;

  invariant(transaction.family === "platon", "not platon family");
...
export { options, PlatONCustomFees as component };
